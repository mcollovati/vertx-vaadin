...
public class TestUtil {
    public static void assertArrays(Object[] actualObjects, Object[] expectedObjects) {
        ...
        for (int i = 0; i < actualObjects.length; i++) {
            ...
            Assert.assertEquals("Item[" + i + "] does not match", expected, actual);
        }
    }
    ...
    public static void assertIterableEquals(Iterable<?> iterable1, Iterable<?> iterable2) {
        ...
        while (i1.hasNext()) {
            Object o1 = i1.next();
            if (!i2.hasNext()) {
                Assert.fail("The second iterable contains fewer items than the first. The object " + o1
                        + " has no match in the second iterable.");
            }
            Object o2 = i2.next();
