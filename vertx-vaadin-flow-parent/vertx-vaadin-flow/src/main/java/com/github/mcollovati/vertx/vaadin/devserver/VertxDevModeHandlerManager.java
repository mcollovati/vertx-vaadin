...
public class VertxDevModeHandlerManager /* extends DevModeHandlerManagerImpl */ {
    ...
    public static void patchViteHandler() {
        try {
            Class.forName("com.vaadin.base.devserver.ViteHandler", false,
                    VertxDevModeHandlerManager.class.getClassLoader());
        } catch (ClassNotFoundException e) {
            LoggerFactory.getLogger(VertxDevModeHandlerManager.class)
                    .debug("ViteHandler not present");
            return;
        }
        ByteBuddyAgent.install();
        new ByteBuddy().redefine(ViteHandler.class)
                .method(ElementMatchers.named("getPathToVaadin")).intercept(// FixedValue.value("/VAADIN/"))
                        MethodDelegation.to(Target.class))
                .make().load(ViteHandler.class.getClassLoader(),
    ...
    private static AbstractDevServerRunner unwrapHandler(
        ...
        throw new IllegalArgumentException(
                "Given DevModeHandler does not extend "
                        + AbstractDevServerRunner.class.getName() + ": "
                        + devModeHandler.getClass().getName());
    }

    public static CompletableFuture<Integer> getDevModeHandlerFuture(
            DevModeHandler devModeHandler) {
        try {
            Field devServerStartFuture = AbstractDevServerRunner.class
                    .getDeclaredField("devServerStartFuture");
            devServerStartFuture.setAccessible(true);
            return ((CompletableFuture<Void>) devServerStartFuture
                    .get(unwrapHandler(devModeHandler)))
                    .thenApply(unused -> devModeHandler.getPort());
        } catch (Exception ex) {
            LoggerFactory.getLogger(VertxDevModeHandlerManager.class)
                    .error("Cannot get DevModHandler future", ex);
