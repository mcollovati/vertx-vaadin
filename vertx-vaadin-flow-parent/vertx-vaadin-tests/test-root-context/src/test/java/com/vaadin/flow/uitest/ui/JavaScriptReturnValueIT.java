...
public class JavaScriptReturnValueIT extends ChromeBrowserTest {
    ...
    private void testCombination(String method, String value, String outcome,
        ...
        for (String target : Arrays.asList("button#clear", inputSelectors.apply(method),
            inputSelectors.apply(value), inputSelectors.apply(outcome), inputSelectors.apply(type), "button#run")) {
        ...
        if ("timeout".equals(type)) {
            try {
                Assert.assertEquals(
                    "Result should not be there immediately for "
                        + combinationId,
                    "Running...", findElement(By.cssSelector("div#status")).getText());

                waitUntil(ExpectedConditions.textToBe(By.cssSelector("div#status"),
                    expectedStatus), 2);
            ...
            Assert.assertEquals("Unexpected result for " + combinationId,
                expectedStatus, findElement(By.cssSelector("div#status")).getText());
        }
    }
